...
/* Ensure text structures respect font layouts safely */
...
[data-testid="stSidebar"] {
    ...
    transform: none !important; /* Forces sidebar to stay on-screen */
    transition: none !important;
}

/* Prevent main dashboard content from covering the sidebar space */
...
.stSlider > div > div > div { background: #00BFFF !important; }
...
# ── MATPLOTLIB DARK THEME & EMOJI SUPPORT ──
plt.rcParams.update({
    ...
    'grid.linestyle':    '--',
    'grid.alpha':        0.5,
    'font.family':       ['Segoe UI Emoji', 'Arial Unicode MS', 'DejaVu Sans', 'sans-serif'],
...
COLORS = {0: '#00BFFF', 1: '#f59e0b', 2: '#ef4444'}
CLUSTER_NAMES = {
    0: "💎 Premium Spenders",
    1: "😐 Low Engagement",
    2: "⚠️ Cash Dependent"
...
with st.sidebar:
    ...
    <div style='text-align:center; padding:16px 0 24px 0;'>
        <div style='font-family:Syne,sans-serif; font-size:1.6rem; font-weight:800;
                    color:#00BFFF; letter-spacing:0.05em;'>
            CreditIQ
        </div>
    ...
    st.markdown("<p style='font-size:0.75rem; color:#475569; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:6px;'>🎛️ Segment Filter</p>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='font-size:0.75rem; color:#475569; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:6px;'>💳 Credit Limit Range</p>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='font-size:0.75rem; color:#475569; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:6px;'>🛒 Spending Range</p>", unsafe_allow_html=True)
    spending_range = st.slider(
        "Spending",
        int(df['PURCHASES'].min()),
    ...
    <div style='font-size:0.72rem; color:#64748b; line-height:2;'>
        <b style='color:#94a3b8;'>Dataset</b><br>8,950 credit card holders<br><br>
        <b style='color:#94a3b8;'>Algorithm</b><br>K-Means (K=3)<br><br>
        <b style='color:#94a3b8;'>Validation</b><br>Silhouette: 0.2510<br>ANOVA p &lt; 0.001<br><br>
        <b style='color:#94a3b8;'>Context</b><br>India Fintech 2026
    </div>
    """, unsafe_allow_html=True)
...
with col1:
    ...
    for at in autotexts:
        at.set_color('#e8eaf0'); at.set_fontsize(10); at.set_fontweight('bold')
    legend_patches = [mpatches.Patch(color=wedge_colors[i], label=counts.index[i]) for i in range(len(counts))]
    ax.legend(handles=legend_patches, loc='lower center', bbox_to_anchor=(0.5, -0.12), ncol=1, fontsize=8)
    ax.set_title('Customer Segments', pad=10)
    fig.tight_layout()
    st.pyplot(fig)
    st.markdown('<div class="insight-box insight-box-yellow">💡 68.3% are low-engagement — biggest untapped revenue opportunity</div>', unsafe_allow_html=True)
...
with col2:
    ...
    fig, ax = plt.subplots(figsize=(5, 4))
    bars = ax.barh(risk_df['Segment'], risk_df['Score'], color=risk_df['Color'], height=0.5, edgecolor='none')
    for bar, score in zip(bars, risk_df['Score']):
        ax.text(score+1, bar.get_y()+bar.get_height()/2, f'{score}/100',
                va='center', fontsize=10, fontweight='bold', color='#e8eaf0')
    ax.set_xlim(0, 110)
    ...
    fig.tight_layout()
    st.pyplot(fig)
    st.markdown('<div class="insight-box insight-box-red">🔴 Cash Dependent scores 87/100 — immediate intervention required</div>', unsafe_allow_html=True)
...
with col3:
    ...
    profile.columns = ['Balance','Purchases','Cash Adv','Credit Lim','Payments','Full Pay%']
    profile['Full Pay%'] = (profile['Full Pay%']*100).round(1).astype(str)+'%'
    
    st.dataframe(profile, width="stretch", height=220)
    ...
    for c in [0,1,2]:
        if c in selected_clusters:
            ...
            <div style='display:flex; justify-content:space-between; align-items:center;
                ...
                <span class='badge {badge_class}'>{RISK_LABEL[c]}</span>
            </div>
            """, unsafe_allow_html=True)
...
for c in selected_clusters:
    ...
    ax.scatter(subset['CREDIT_LIMIT'], subset['PURCHASES'],
               c=COLORS[c], label=CLUSTER_NAMES[c], alpha=0.45, s=25, edgecolors='none')
...
<div style='display:grid; grid-template-columns:1fr 1fr 1fr; gap:12px; margin-top:8px;'>
    ...
    <div class='insight-box insight-box-red'>⚠️ <b>Cash Dependent:</b> Mid-high limit but low purchases → using cash advance instead</div>
</div>
""", unsafe_allow_html=True)
...
with col1:
    ...
    hmap_cols = ['BALANCE','PURCHASES','CASH_ADVANCE','CREDIT_LIMIT','PAYMENTS','MINIMUM_PAYMENTS']
    hmap_data = df_f.groupby('Segment')[hmap_cols].mean()
    hmap_norm = (hmap_data - hmap_data.min()) / (hmap_data.max() - hmap_data.min() + 1e-9)
    
    hmap_norm.index = [idx.split(' ', 1)[1] for idx in hmap_norm.index]
    
    fig, ax = plt.subplots(figsize=(7, 3.5))
    sns.heatmap(hmap_norm, annot=True, fmt='.2f', cmap='YlOrRd', ax=ax,
                linewidths=0.5, linecolor='#0a0e1a', annot_kws={"size": 9, "color": "#0a0e1a"})
...
with col2:
    ...
    fig, ax  = plt.subplots(figsize=(7, 3.5))
    x = np.arange(len(bar_data)); w = 0.25
    ax.bar(x-w, bar_data['PURCHASES'],    w, label='Purchases',    color='#00BFFF', alpha=0.85)
    ax.bar(x,   bar_data['PAYMENTS'],     w, label='Payments',     color='#10b981', alpha=0.85)
    ax.bar(x+w, bar_data['CASH_ADVANCE'], w, label='Cash Advance', color='#ef4444', alpha=0.85)
    ax.set_xticks(x)
    ax.set_xticklabels([s.split(' ',1)[1] for s in bar_data.index], fontsize=8, rotation=10)
...
with col1:
    ...
    feat_cols = [c for c in df.columns if c not in ['Cluster','Segment','Risk_Score','Risk_Label']]
    scaled    = StandardScaler().fit_transform(df[feat_cols])
    ...
    fig.tight_layout()
    st.pyplot(fig)
    st.markdown('<div class="insight-box">🔬 Clusters spatially separated — confirms strong segmentation. Silhouette Score: <b>0.2510</b></div>', unsafe_allow_html=True)
...
with col2:
    ...
    feature_select = st.selectbox("Select Feature:",
        ['PURCHASES','CASH_ADVANCE','BALANCE','CREDIT_LIMIT',
         'PAYMENTS','MINIMUM_PAYMENTS','PRC_FULL_PAYMENT','TENURE'])
    fig, ax = plt.subplots(figsize=(7, 4.5))
    for c in selected_clusters:
        ax.hist(df_f[df_f['Cluster']==c][feature_select], bins=40,
                alpha=0.65, label=CLUSTER_NAMES[c], color=COLORS[c], edgecolor='none')
    ax.set_xlabel(feature_select); ax.set_ylabel("Customer Count")
...
val_cols = ['BALANCE','PURCHASES','CASH_ADVANCE','CREDIT_LIMIT','PAYMENTS','MINIMUM_PAYMENTS']
vcols = st.columns(len(val_cols))
for i, col in enumerate(val_cols):
    f, p = scipy_stats.f_oneway(df[df['Cluster']==0][col], df[df['Cluster']==1][col], df[df['Cluster']==2][col])
    with vcols[i]:
        st.markdown(f"""
        <div style='background:#1a2235; border-radius:10px; padding:12px; text-align:center;
                    border:1px solid rgba(0,191,255,0.1);'>
            <div style='font-size:0.65rem; color:#475569; text-transform:uppercase; letter-spacing:0.06em;'>{col.replace("_"," ")}</div>
            <div style='font-family:Syne,sans-serif; font-size:1.1rem; color:#00BFFF; font-weight:700; margin:4px 0;'>F={f:,.0f}</div>
            <div style='font-size:0.72rem; color:#10b981;'>p &lt; 0.001 ✅</div>
        </div>
        """, unsafe_allow_html=True)
...
with b1:
    ...
    <div style='background:linear-gradient(135deg,#0d2137,#0f2a45); border:1px solid #00BFFF44;
        ...
        <div style='font-family:Syne,sans-serif; font-size:1rem; font-weight:700; color:#00BFFF;'>💎 Premium Spenders</div>
        <div style='font-size:0.75rem; color:#475569; margin:4px 0 12px 0;'>Cluster 0 · 1,275 customers · <span style='color:#10b981;'>LOW RISK</span></div>
        ...
        <div style='margin-top:14px; font-size:0.8rem; color:#64748b; font-weight:600; text-transform:uppercase; letter-spacing:0.06em;'>Strategy</div>
        <div style='font-size:0.82rem; color:#94a3b8; line-height:1.9; margin-top:6px;'>
            ✅ Premium rewards & loyalty points<br>✅ Proactive credit limit increases<br>
            ✅ Exclusive travel & lifestyle perks<br>✅ Dedicated relationship manager
        </div>
        <div style='margin-top:14px; background:rgba(0,191,255,0.08); border-radius:8px; padding:8px 10px; font-size:0.78rem; color:#00BFFF;'>
...
with b2:
    ...
    <div style='background:linear-gradient(135deg,#1a1500,#241d00); border:1px solid #f59e0b44;
        ...
        <div style='font-family:Syne,sans-serif; font-size:1rem; font-weight:700; color:#f59e0b;'>😐 Low Engagement</div>
        <div style='font-size:0.75rem; color:#475569; margin:4px 0 12px 0;'>Cluster 1 · 6,114 customers · <span style='color:#f59e0b;'>MEDIUM RISK</span></div>
        <div style='font-size:0.82rem; color:#94a3b8; line-height:1.8;'>
        ...
        <div style='margin-top:14px; font-size:0.8rem; color:#64748b; font-weight:600; text-transform:uppercase; letter-spacing:0.06em;'>Strategy</div>
        <div style='font-size:0.82rem; color:#94a3b8; line-height:1.9; margin-top:6px;'>
            📢 Personalised cashback campaigns<br>📢 Gamify spending milestones<br>
            📢 Targeted EMI & BNPL offers<br>📢 Re-engagement email journeys
        </div>
        <div style='margin-top:14px; background:rgba(245,158,11,0.08); border-radius:8px; padding:8px 10px; font-size:0.78rem; color:#f59e0b;'>
...
with b3:
    ...
    <div style='background:linear-gradient(135deg,#1a0808,#240d0d); border:1px solid #ef444444;
        ...
        <div style='font-family:Syne,sans-serif; font-size:1rem; font-weight:700; color:#ef4444;'>⚠️ Cash Dependent</div>
        <div style='font-size:0.75rem; color:#475569; margin:4px 0 12px 0;'>Cluster 2 · 1,561 customers · <span style='color:#ef4444;'>HIGH RISK</span></div>
        <div style='font-size:0.82rem; color:#94a3b8; line-height:1.8;'>
        ...
        <div style='margin-top:14px; font-size:0.8rem; color:#64748b; font-weight:600; text-transform:uppercase; letter-spacing:0.06em;'>Strategy</div>
        <div style='font-size:0.82rem; color:#94a3b8; line-height:1.9; margin-top:6px;'>
            🔴 Flag for default risk monitoring<br>🔴 Offer debt restructuring plans<br>
            🔴 Financial wellness counseling<br>🔴 Gradually limit cash advance
        </div>
        <div style='margin-top:14px; background:rgba(239,68,68,0.08); border-radius:8px; padding:8px 10px; font-size:0.78rem; color:#ef4444;'>
